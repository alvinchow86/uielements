...
(function() {
  ...
  function initMainNav() {
    $(".main-nav-item.has-subnav").each(function() {
      var $this = $(this);
      var link = $this.children(".main-nav-link");
      var subnav = $this.children(".main-subnav");
  ...
  /* based on http://www.designchemical.com/blog/index.php/jquery/add-default-text-to-form-fields-using-jquery/ */


  function initUserLocation() {
    UserLocation.enableLocationTooltip($("#user-location-link"), {
      x: -50,
      y: 10
    });

    // set listener for location events
    UserLocationEventDispatcher.addListenerCallback(function(geolocation, args) {
      UserLocation.updateLocation(geolocation.location);
    }, 'userlocation');

  }

  function initChangeLocationLinks() {
    UserLocation.enableLocationTooltip($(".change-location-link"), {
      //x: -50,
      //y: 10
    });
  }
...
$(document).ready(function() {
  ...
  var formcontent = "<div class='location_tooltip'><span class='text'>Enter a new location:</span> <div class='searchbox'><input name='new_location' id='new_location' type='text' onkeypress='if(event.keyCode==13){UserLocation.changeLocation();}'> <a class='button' href='#' onclick='UserLocation.changeLocation(); return false;'> OK </a> <div class='floatclear'></div> </div>  <a href='#' onclick='UserLocation.clearLocation(); return false;' class='clear_location'> (clear location) </a></div>";
